...
async function getTypeAdjustments(req, res){
        try {
            const resultados = await ejecutarVistaTools('vw_ct_type_adjustment');
            if(resultados.length > 0){
                res.status(201).json({valido: 1, result: resultados});
            } else {
                res.status(500).json({valido: 0, message: "Was an error, please, try again"});
...
async function setTypeAdjustment(req, res) {
    try {
        let IDUser = await catchUserLogged(req);    
        const body = req.body;
        const resultados = await ejecutarStoredProcedure('sp_setTypeAdjustment', [
            body.Idtypeadjustment,
            body.ShortDesc_type_adjustment,
            body.LargeDesc_type_adjustment,
            body.status,
            IDUser.IDUser
        ]);
        if (resultados.length > 0) {
...
// call db_portafolio_v2.sp_setTypeAdjustment(4, 'update', 'prueba update', 0, 32);
